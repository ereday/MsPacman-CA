...
public class Maze {
    ...
    public void calculateDecayValue(){

    }

	public class NodeValue{
        // value is either of @,P,p,G,E,e
        protected String value=null;
        // decay value is initally 1
        protected float decayValue;


        public NodeValue(float decayValue ){
	...
	public class MazeNode {
        ...
        protected NodeValue nodeValue=null;

        protected Random rndNumber=new Random();
		public MazeNode(int index) {
			this.index = index;
            this.nodeValue=new NodeValue(1);
		}
        ...
        public float calculateDecayValue(){
            float pd=this.getNodeValue().getDecayValue();
            float randomNumber=this.uniform(-0.4f, 0.4f,rndNumber);
            float result=pd-(pd/7)+(randomNumber*(pd/1000));
            return result;
        }
        ...
        /**
         *  Check domination rules and pass cell's value to neighbor nodes
         *  if rules are okey
         */
        public void dominate(){
			String nodeValue=this.getNodeValue().getValue();
                for(int i=0;i<this.neighbours.length;i++){
                    if(this.neighbours[i]==null)
                        continue;

                    String neighborValue=this.neighbours[i].getNodeValue().getValue();
                    if(nodeValue.equals("p")){
                        if(neighborValue.equals("e")){
                            float newDecayValue=this.calculateDecayValue();
                            if(newDecayValue>=1.0E-42){
                            this.neighbours[i].getNodeValue().setValue("p");
                            this.neighbours[i].getNodeValue().setDecayValue(newDecayValue);
                            }
                        }
                    }
                    else if(nodeValue.equals("E")){
                        if((!neighborValue.equals("E")) && (!neighborValue.equals("@"))){
                            float newDecayValue=this.calculateDecayValue();
                            if(newDecayValue>=1.0E-42 && !(neighborValue.equals("G") && this.neighbours[i].getNodeValue().getDecayValue()>this.getNodeValue().getDecayValue() )){
                                this.neighbours[i].getNodeValue().setValue("E");
                                this.neighbours[i].getNodeValue().setDecayValue(newDecayValue);
                            }

                        }
                    }
                    else if(nodeValue.equals("P")){
                        if(neighborValue.equals("p") || neighborValue.equals("G") || neighborValue.equals("e")){
                            float newDecayValue=this.calculateDecayValue();
                            this.neighbours[i].getNodeValue().setValue("P");
                            this.neighbours[i].getNodeValue().setDecayValue(newDecayValue);
                        }
                    }
                    else if(nodeValue.equals("G")){
                       if(neighborValue.equals("p") || neighborValue.equals("e")|| (neighborValue.equals("G") && this.neighbours[i].getNodeValue().getDecayValue()<=this.getNodeValue().getDecayValue() )){ //|| (neighborValue.equals("G") && this.neighbours[i].getNodeValue().getDecayValue()<=this.getNodeValue().getDecayValue())
                           float newDecayValue=this.calculateDecayValue();
                          // if(newDecayValue>=1.0E-42) {
                               this.neighbours[i].getNodeValue().setValue("G");
                               this.neighbours[i].getNodeValue().setDecayValue(newDecayValue);
                           //}
                       }
                    }
        ...
        public boolean isNodeValueEmpty(){
            if(this.nodeValue.getValue().equals("e")){
                return true;
            }
            return false;
        }
        public boolean isNodeValuePacman(){
            if(this.nodeValue.getValue().equals("@")){
                return true;
            }
            return false;
        }
        ...
        public boolean isEmpty(){
            if(this.pacman() || this.powerPill() || this.pill() || this.ghost() || this.ghostEdible()){
                return false;
            }
            else{
                return true;
        ...
        public void setDecayValue(float i){

            this.nodeValue.decayValue=i;
        }

        /**
         * set's what is in the node for the next move [domination will be done based on these values]
         */
        public void setNodeValue(){
            if(this.powerPill()){
                this.nodeValue.value="P";
            }
            else if(this.pacman()){
                this.nodeValue.value="@";
            }
            else if(this.ghostEdible()){
                this.nodeValue.value="E";
            }
            else if(this.ghost()){
                this.nodeValue.value="G";
            }
            else if(this.pill()){
                this.nodeValue.value="p";
            }
