...
public class _G_ extends G
{
	...
	public void newGame(GameConfig config)
	{
		...
		pills=new BitSet(getNumberPills()); // check whether pills are consumed or not
		pills.set(0,getNumberPills());     // initially all exist and so true
		powerPills=new BitSet(getNumberPowerPills()); // check whether power pills are consumed or not
